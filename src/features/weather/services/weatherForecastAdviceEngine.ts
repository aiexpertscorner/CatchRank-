...
export interface FishingWeatherSnapshotInput {
  discipline: Exclude<FishingDiscipline, 'algemeen'>;
  hourly: OpenWeatherHourly[];
  daily?: OpenWeatherDaily[];

  // Spot / context fields already useful for your static engine
  waterType?: WaterType;
  depthBand?: DepthBand;
  clarity?: WaterClarity;
  vegetation?: VegetationLevel;

  // For bank/wind advice
  preferredShoreBearing?: number; // direction from bank toward water, 0..359
  oppositeShoreBearing?: number;

  // Optional additional context
  month?: number; // 1..12
  isNightFishing?: boolean;
  baseWaterTemp?: number;
}
...
export interface ForecastParameterAdvice {
  key:
    | 'temperature'
    | 'windSpeed'
    | 'windDirection'
...
export interface HourlyFishingAdvice {
  dt: number;
  score: number;
  band: 'poor' | 'fair' | 'good' | 'strong' | 'peak';
  confidence: number;
  reasons: string[];
  impacts: ForecastParameterAdvice[];
  primaryAdvice: string[];
  shoreAdvice?: ShoreAdvice;
  contextForStaticAdvice: AdviceContext;
}

export interface FishingForecastAdviceOutput {
  overallSummary: string;
  sessionAdvice: string[];
  parameterHighlights: ForecastParameterAdvice[];
  hourlyScores: HourlyFishingAdvice[];
  bestHours: HourlyFishingAdvice[];
  bestNow?: HourlyFishingAdvice;
  topWindows: Array<{ startDt: number; endDt: number; avgScore: number }>;
}
...
function scoreToBand(score: number): HourlyFishingAdvice['band'] {
...
/* ==========================================================================
   Mapping weather -> static AdviceContext
   ========================================================================== */

export function mapTempToTemperatureBand(temp: number): TemperatureBand {
  if (temp < 8) return 'koud';
  if (temp <= 16) return 'gematigd';
  return 'warm';
}

export function estimatePressureTrend(hourly: OpenWeatherHourly[], index: number): PressureTrend {
...
export function buildAdviceContextFromHourly(
  input: FishingWeatherSnapshotInput,
  ...
  return {
    ...
    temperatureBand: mapTempToTemperatureBand(hour.temp),
    pressureTrend: estimatePressureTrend(input.hourly, index),
  } as AdviceContext;
}

/* ==========================================================================
   Parameter scoring blocks
   ========================================================================== */

function getMoonPhaseForHour(input: FishingWeatherSnapshotInput, hourUnix: number): number | undefined {
  if (!input.daily?.length) return undefined;
  const targetDate = new Date(hourUnix * 1000).toDateString();
  const match = input.daily.find((d) => new Date(d.dt * 1000).toDateString() === targetDate);
  return match?.moon_phase;
}

function scoreTemperature(
  temp: number,
  discipline: FishingWeatherSnapshotInput['discipline'],
  month?: number,
  waterType?: WaterType,
): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];
...
function scoreWindSpeed(speed: number, discipline: FishingWeatherSnapshotInput['discipline']): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];
...
function scoreWindDirection(
  ...
  preferredShoreBearing: number | undefined,
  oppositeShoreBearing: number | undefined,
  discipline: FishingWeatherSnapshotInput['discipline'],
): { impact: ForecastParameterAdvice; shoreAdvice?: ShoreAdvice } {
  ...
  if (discipline === 'karper') {
    if (isWindOnPreferred) {
      ...
      tips.push('Vis niet altijd pal in de branding; het eerste drop-off kan beter zijn.');
    } else if (isCrossPreferred) {
      preferredBankScore = 2;
      oppositeBankScore = 1;
      summary = 'Schuine zijwind: bruikbaar, maar minder uitgesproken oevervoordeel.';
      tips.push('Focus op windgedreven hoeken en banen, niet alleen op één rechte oever.');
    } else {
...
function scorePressure(pressure?: number): ForecastParameterAdvice {
  if (pressure == null) {
    return {
      key: 'pressure',
      label: 'Luchtdruk',
...
function scorePressureTrend(trend: PressureTrend, discipline: FishingWeatherSnapshotInput['discipline']): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];
...
function scoreUV(uvi = 0, hour = 12, discipline: FishingWeatherSnapshotInput['discipline']): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];

  if (isPrimeLowLight(hour)) scoreImpact += 3;
...
function scoreCloudCover(clouds = 0, discipline: FishingWeatherSnapshotInput['discipline']): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];
...
function scorePrecipitation(pop = 0, rain1h = 0, discipline: FishingWeatherSnapshotInput['discipline']): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];
...
function scoreHumidity(humidity?: number): ForecastParameterAdvice {
  if (humidity == null) {
    return {
      key: 'humidity',
...
function scoreVisibility(visibility?: number, clarity?: WaterClarity, discipline?: FishingWeatherSnapshotInput['discipline']): ForecastParameterAdvice {
  if (visibility == null) {
    return {
      key: 'visibility',
      label: 'Zicht',
...
function scoreMoonPhase(moonPhase?: number, isNightFishing?: boolean): ForecastParameterAdvice {
  if (moonPhase == null) {
    return {
      key: 'moonPhase',
      label: 'Maanfase',
  ...
  if (!isNightFishing) {
    ...
    scoreImpact = 4;
    summary = 'Veel maanlicht: kan oriëntatie en activiteit tijdens de nacht helpen.';
    tips.push('Open-water nachtstekken met zichtlijnen kunnen sterker zijn.');
  } else if (moonLightPct > 35) {
    scoreImpact = 2;
    summary = 'Gemiddeld maanlicht: lichte positieve invloed voor nachtvissen.';
  } else {
    scoreImpact = -1;
    summary = 'Donkere maan: minder licht, vis kan lokaler of voorzichtiger actief zijn.';
...
function scoreDaypart(hour: number, discipline: FishingWeatherSnapshotInput['discipline'], temp: number): ForecastParameterAdvice {
  let scoreImpact = 0;
  let summary = '';
  const tips: string[] = [];
  ...
  if (isPrimeLowLight(hour)) {
    ...
    summary = discipline === 'karper'
      ...
      : 'Nacht is alleen selectief interessant voor roofvis; overdag en schemer zijn vaak beter.';
  } else {
    scoreImpact = 1;
...
function scoreGusts(gust?: number, windSpeed?: number): ForecastParameterAdvice {
...
/* ==========================================================================
   Advice composition helpers
   ========================================================================== */

function buildPrimaryAdvice(impacts: ForecastParameterAdvice[], discipline: FishingWeatherSnapshotInput['discipline']): string[] {
  ...
  for (const item of positives.slice(0, 3)) item.tips.slice(0, 2).forEach((t) => tips.add(t));
  for (const item of negatives.slice(0, 3)) item.tips.slice(0, 2).forEach((t) => tips.add(t));
...
function buildReasons(impacts: ForecastParameterAdvice[]): string[] {
  return impacts
...
export function getTopFishingWindows(
  ...
  for (let i = 0; i < hourlyScores.length - 1; i++) {
    const current = hourlyScores[i];
    const next = hourlyScores[i + 1];

    if (current.score >= 65 && next.score >= 65) {
      const block = [current, next];
      let j = i + 2;

      while (j < hourlyScores.length && hourlyScores[j].score >= 62) {
        block.push(hourlyScores[j]);
        j++;
      }
      ...
      windows.push({
        ...
        avgScore: round(block.reduce((sum, h) => sum + h.score, 0) / block.length, 0),
      });
...
export function getFishingForecastAdvice(
  input: FishingWeatherSnapshotInput,
  options?: { limitHours?: number },
): FishingForecastAdviceOutput {
  const limitHours = options?.limitHours ?? 48;
  const hourly = input.hourly.slice(0, limitHours);

  const hourlyScores: HourlyFishingAdvice[] = hourly.map((hour, index) => {
    const derivedContext = buildAdviceContextFromHourly(input, hour, index);
    ...
    const pressureTrend = estimatePressureTrend(hourly, index);
    const pressureImpact = scorePressure(hour.pressure);
    const pressureTrendImpact = scorePressureTrend(pressureTrend, input.discipline);
    ...
    const moonImpact = scoreMoonPhase(
      getMoonPhaseForHour(input, hour.dt),
      input.isNightFishing && isNight(getHour(hour.dt)),
    );
    ...
    const impacts: ForecastParameterAdvice[] = [
      tempImpact,
      windSpeedImpact,
    ...
    return {
      ...
      contextForStaticAdvice: derivedContext,
    };
  });
  ...
  const topImpactMap = new Map<string, ForecastParameterAdvice>();
  ...
  return {
    ...
    bestHours,
    bestNow,
    topWindows: getTopFishingWindows(hourlyScores, 3),
  };
}

/* ==========================================================================
   UI helpers you can directly use
   ========================================================================== */

export function getBestFishingHour(input: FishingWeatherSnapshotInput): HourlyFishingAdvice | undefined {
  return getFishingForecastAdvice(input).bestHours[0];
}

export function getNowFishingAdvice(input: FishingWeatherSnapshotInput): HourlyFishingAdvice | undefined {
  return getFishingForecastAdvice(input, { limitHours: 12 }).hourlyScores[0];
}

/* ==========================================================================
   Example mapper from OpenWeather One Call snapshot shape
   ========================================================================== */

export interface OpenWeatherOneCallLike {
  hourly?: OpenWeatherHourly[];
  daily?: OpenWeatherDaily[];
}

export function buildFishingWeatherSnapshotInput(params: {
  oneCall: OpenWeatherOneCallLike;
  ...
  isNightFishing?: boolean;
  baseWaterTemp?: number;
}): FishingWeatherSnapshotInput {
  return {
    discipline: params.discipline,
    hourly: params.oneCall.hourly ?? [],
    daily: params.oneCall.daily ?? [],
    waterType: params.waterType,
    depthBand: params.depthBand,
    clarity: params.clarity,
    vegetation: params.vegetation,
    preferredShoreBearing: params.preferredShoreBearing,
    oppositeShoreBearing: params.oppositeShoreBearing,
    month: params.month ?? new Date().getMonth() + 1,
    isNightFishing: params.isNightFishing,
    baseWaterTemp: params.baseWaterTemp,
  };
}