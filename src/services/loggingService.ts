...
export const loggingService = {
  ...
  /**
   * Complete a draft catch.
   */
  async completeCatch(catchId: string, data: Partial<Catch>): Promise<void> {
    ...
    await updateDoc(docRef, {
      ...data,
      status: incompleteFields.length === 0 ? 'complete' : 'draft',
      incompleteFields,
      xpEarned,
    });
  },
  ...
  calculateIncompleteFields(data: Partial<Catch>): string[] {
    const required = ['species', 'weight', 'length', 'spotId'];
