...
export interface ProgressBarProps extends React.HTMLAttributes<HTMLDivElement> {
  value: number;
  max?: number;
  variant?: 'brand' | 'success' | 'blue' | 'aqua' | 'warning' | 'danger';
  size?: 'xs' | 'sm' | 'md' | 'lg';
  showLabel?: boolean;
  label?: string;
}
...
export const ProgressBar: React.FC<ProgressBarProps> = ({ 
  ...
  const variants = {
    brand: 'bg-brand shadow-[0_0_10px_rgba(244,194,13,0.3)]',
    success: 'bg-success shadow-[0_0_10px_rgba(41,195,106,0.3)]',
    blue: 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.3)]',
    aqua: 'bg-aqua shadow-[0_0_10px_rgba(0,255,255,0.3)]',
    warning: 'bg-warning shadow-[0_0_10px_rgba(240,168,58,0.3)]',
    danger: 'bg-danger shadow-[0_0_10px_rgba(224,86,86,0.3)]',
  };
    ...
    <div className={cn("w-full space-y-1.5", className)} {...props}>
      {(showLabel || label) && (
        <div className="flex justify-between items-center px-0.5">
          {label && <span className="text-[9px] font-black text-text-muted uppercase tracking-widest">{label}</span>}
          {showLabel && <span className="text-[9px] font-black text-text-primary uppercase tracking-widest">{Math.round(percentage)}%</span>}
        </div>
      )}
      <div className={cn("w-full bg-surface-soft rounded-full overflow-hidden", sizes[size])}>
        <div 
