    ...
    <aside className={cn(
      ...
      <div className={cn("p-6 flex items-center justify-between", isCollapsed && "justify-center")}>
        ...
        <button 
          ...
          className="absolute -right-3 top-12 w-6 h-6 bg-surface border border-border-subtle rounded-full flex items-center justify-center text-text-muted hover:text-accent hover:border-accent transition-all shadow-sm z-50"
        >
        ...
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) => cn(
              "nav-item relative group transition-all duration-300",
              isActive ? "bg-accent text-white shadow-premium-accent/30 shadow-lg" : "hover:bg-surface-soft text-text-muted hover:text-text-primary",
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {!isCollapsed && isActive && <ChevronRight className="ml-auto w-4 h-4 opacity-50" />}
      ...
      <div className="p-4 border-t border-border-subtle space-y-2">
        ...
        >
          {({ isActive }) => (
            <>
              ...
              {!isCollapsed && <span className="font-bold tracking-tight">Profiel</span>}
              {isCollapsed && (
                <div className="absolute left-full ml-4 px-3 py-2 bg-accent text-white text-[10px] font-black uppercase tracking-widest rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-xl">
                  Profiel
                </div>
        ...
        <button
          onClick={logout}
          className={cn(
            "w-full nav-item relative group hover:bg-danger/10 hover:text-danger",
            isCollapsed && "justify-center px-0"
          )}
        >
          <LogOut className="w-5 h-5 flex-shrink-0 text-text-muted group-hover:text-danger" />
          {!isCollapsed && <span className="font-semibold tracking-tight">Uitloggen</span>}
          {isCollapsed && (
            <div className="absolute left-full ml-4 px-3 py-2 bg-danger text-white text-xs font-bold rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-xl">
              Uitloggen
            </div>
          )}
        </button>
