...
const getInvitedIds = (session: Partial<Session>): string[] =>
  (session as any).invitedUserIds || [];
...
export const ParticipantInviteModal: React.FC<ParticipantInviteModalProps> = ({
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const invitedIds = getInvitedIds(session);

      const filtered = results.filter(
        (u) => !participantIds.includes(u.uid) && !invitedIds.includes(u.uid)
      );
      ...
      >
        ...
        <div className="p-6 sm:p-8 border-b border-border-subtle bg-surface-soft/30">
          <Input
            ...
            icon={
              isSearching ? (
                <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 text-water animate-spin" />
              ) : (
        ...
        <div className="p-4 sm:p-6 overflow-y-auto no-scrollbar flex-1 space-y-3">
          {searchTerm.length < 2 ? (
            ...
            <div className="py-12 text-center space-y-4">
              ...
              <p className="text-sm text-text-muted font-medium">
                Geen gebruikers gevonden met deze naam.
              </p>
            </div>
          )}
        </div>
